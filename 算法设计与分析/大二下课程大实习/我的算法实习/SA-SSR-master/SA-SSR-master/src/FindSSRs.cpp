...
{
	sem_wait(&(this->d)); // acquire lock for finished threads 获取资源，资源不够则阻塞
	++this->finished_threads;
	sem_post(&(this->d)); // release lock for finished threads 释放资源
}
...
void FindSSRs::processInput() // produce
{
	...
	species1_in_file.open(this->args->getSpecies1FastaFileName().c_str());//打开输入文件
	...
	string sequence = "";
	string line = "";
	while (getline(species1_in_file, line))//读取文件species1_in_file一行，放入line中
	{
		if (line.size() > 0)
		{
			if (line[0] != '>')//不是基因序列名
...
void FindSSRs::processSequence(const string &header, string &sequence)
{
	...
	if ( (sequence.size() >= this->args->getMinSequenceLength()) && (sequence.size() <= this->args->getMaxSequenceLength()) )
	{
		...
		if (!this->args->isCharsToIgnore())//不是需要被忽略的序列
		{
			starts.push_back(0);
